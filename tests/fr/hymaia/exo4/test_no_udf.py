import unittest
from src.fr.hymaia.exo4.no_udf import calculate_total_price_per_category_per_day, \
...
class TestNoUdf(unittest.TestCase):
    ...
    def test_calculate_total_price_per_category_per_day_last_30_days(self):
        ...
        actual_list = [(row["id"], row["date"], row["category"], row["price"], row["category_name"],
                        row["total_price_per_category_per_day_last_30_days"]) for row in actual.collect()]
        ...
        expected_list = [(row["id"], row["date"], row["category"], row["price"], row["category_name"],
                          row["total_price_per_category_per_day_last_30_days"]) for row in df_expected.collect()]
    ...
    def test_calculate_total_price_per_category_per_day(self):
        ...
        actual_list = [(row["id"], row["date"], row["category"], row["price"], row["category_name"],
                        row["total_price_per_category_per_day"]) for row in actual.collect()]

        df_expected = spark.createDataFrame(
            [
                (0, "2019-02-13", 4, 12.0, "food", 57.0),
                (0, "2019-02-16", 6, 40.0, "furniture", 143.),
        ...
        expected_list = [(row["id"], row["date"], row["category"], row["price"], row["category_name"],
                          row["total_price_per_category_per_day"]) for row in df_expected.collect()]
